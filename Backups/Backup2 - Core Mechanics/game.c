...
void updatePlayer() {
    ...
    if (!grounded) {
        ...
        if (BUTTON_HELD(BUTTON_UP) && jumping && !jumpThud) {
            ...
            // }
            // // case where you are falling without ever having jumped
            // else {

            // }
            // this 'thud' is for hitting head on ceiling
            // TODO - get rid of line below to enable AUTOMATIC LEDGE JUMPING - one of the 'cheats'
            //  || !BUTTON_PRESSED(BUTTON_UP)
            if (jumpThud) {
                yVel = (GRAVITY * framesInAir);
            }
            // default if let go of key. This allows for brief hopping
