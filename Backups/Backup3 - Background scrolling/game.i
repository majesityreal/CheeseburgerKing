...
void drawGame() {
    ...
    if (hOff >= 256) {
        ...
        if (28 + (hScreenCounter % 4) == 31) {
            DMANow(3, map1Map, &((screenblock *)0x6000000)[28], 4096 / 2);
            (*(volatile unsigned short *)0x400000A) = ((0) << 2) | ((29) << 8) | (1 << 14) | (0 << 7);
        }
        else {
            (*(volatile unsigned short *)0x400000A) = ((0) << 2) | ((28 + (hScreenCounter % 4)) << 8) | (1 << 14) | (0 << 7);
        }
    ...
    if (gTimer % player.movementCycle == 0) {
        ...
        if((~((*(volatile unsigned short *)0x04000130)) & ((1 << 4)))
            ...
            && !checkCollision(player.worldCol + player.width + player.cdel, player.worldRow + player.height - 1)) {
                ...
                player.worldCol += player.cdel;
                if (hOff <= 256 && (player.worldCol - hOff > (240 / 2))) {
                    hOff += player.cdel;
                }
        }
    }
...
void drawFont() {
    ...
    int d1 = abs(hOff) % 10;
        ...
        shadowOAM[shadowOAMIndex].attr1 = (0x1FF & (40)) | (0 << 14);
