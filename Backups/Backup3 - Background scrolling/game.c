...
// stores all the maps [x][y], where x is the maps, and y is the length of each of these maps
...
void drawGame() {
    ...
    if (hOff >= 256) {
        // keeps it to switching between 4 screenblocks, because we have two BG_SIZE_WIDE
        hScreenCounter += 1;
        // changes what we load in for the background
        if (28 + (hScreenCounter % 4) == 31) {
            DMANow(3, map1Map, &SCREENBLOCK[28], map1MapLen / 2);
            REG_BG1CNT = BG_CHARBLOCK(0) | BG_SCREENBLOCK(29) | BG_SIZE_WIDE | BG_4BPP;
        }
        else {
            REG_BG1CNT = BG_CHARBLOCK(0) | BG_SCREENBLOCK(28 + (hScreenCounter % 4)) | BG_SIZE_WIDE | BG_4BPP;
        }    
        ...
        // loading in new collision map every 'odd', in between chunks of wide BGs.
        if (hScreenCounter % 2 == 1) {
            if (hOff >= 128) {
                // include switch statement here for future, with lots of backgrounds
                switch (hScreenCounter) {
                    case 1:
    ...
    if (gTimer % player.movementCycle == 0) {
        ...
        if(BUTTON_HELD(BUTTON_RIGHT) 
            ...
            // if (player.worldCol <= MAPWIDTH + SCREENWIDTH - 30) {
                player.worldCol += player.cdel;
                if (hOff <= 256 && (player.worldCol - hOff > (SCREENWIDTH / 2))) {
                    hOff += player.cdel;
                }
        }
    }
...
void drawFont() {
    ...
    int d1 = abs(hOff) % 10;
        shadowOAM[shadowOAMIndex].attr0 = (ROWMASK & 0) | ATTR0_SQUARE;
        shadowOAM[shadowOAMIndex].attr1 = (COLMASK & (40)) | ATTR1_TINY;
